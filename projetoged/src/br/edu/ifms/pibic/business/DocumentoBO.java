...
import br.edu.ifms.pibic.util.OfficeUtil;
...
public class DocumentoBO {
				...
				if (subDiretorio != null && subDiretorio.exists()) {
					if (subDiretorio.isDirectory()) {
						...
						varreSubdiretorio(caminhoSubdiretorio);
							...
							.equalsIgnoreCase("pdf") || pegaExtensaoArquivo(subDiretorio)
							.equalsIgnoreCase("doc") || pegaExtensaoArquivo(subDiretorio)
							.equalsIgnoreCase("docx") || 
							pegaExtensaoArquivo(subDiretorio)
							.equalsIgnoreCase("xls") || pegaExtensaoArquivo(subDiretorio)
	...
	private boolean salvaDocumento(File arquivo) {
		...
		try {
			...
			if(pegaExtensaoArquivo(arquivo).equals("pdf")){
				numeroPaginas = PDFUtil.pegaNumeroPaginas(arquivo);
				conteudoTextual = PDFUtil.pegaTextoPDF(arquivo);
				
			}else if(pegaExtensaoArquivo(arquivo).equals("doc")){
				conteudoTextual = OfficeUtil.pegaTextoWordDOC(arquivo);
				
			}else if(pegaExtensaoArquivo(arquivo).equals("docx")){
				conteudoTextual = OfficeUtil.pegaTextoWordDOCX(arquivo);
			
			}else if(pegaExtensaoArquivo(arquivo).equals("xls")){
				conteudoTextual = OfficeUtil.pegaTextoExcelXLS(arquivo);

			}else if(pegaExtensaoArquivo(arquivo).equals("xlsx")){
				conteudoTextual = OfficeUtil.pegaTextoExcelXLSX(arquivo);
			}
			...
			documento.setExtensao("."+pegaExtensaoArquivo(arquivo));			
	...
	private int salvaPaginasDoDocumento(Documento documento, File arquivo) {
		...
		int contador = 0;
		File arquivoPagina = arquivo;
		for (int i = 1; i <= documento.getNumeroPaginas(); i++) {
			
			if(pegaExtensaoArquivo(arquivo).equals("pdf")){
				arquivoPagina = PDFUtil.pegaPaginaDocumento(arquivo, i);
			}
